...
const buttonVariants = cva(
  'inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0',
  {
    variants: {
      variant: {
        // Primary: medifly-teal background, white text
        default: 'bg-medifly-teal text-white hover:bg-medifly-teal/90',
  ...
  return (
    <textarea
      className={cn(
        ...
        'flex min-h-20 w-full rounded-md border border-gray-400 bg-white px-3 py-2 text-base ring-offset-white placeholder:text-gray-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-medifly-teal focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 md:text-sm text-black',
        className,
      )}
      ref={ref}
      {...props}
...
function PureSuggestedActions({
  ...
  const suggestedActions = [
    {
      title: 'Rehabilitation hospital for stroke patients',
      label: 'in Singapore for heart check-up',
      action: 'Find rehabilitation hospital for stroke patients',
    },
    {
      title: 'Compare dental',
      label: 'treatments in Malaysia',
      action: 'Compare dental treatments in Malaysia',
    },
    {
      title: 'Emergency medical',
      label: 'services in Bangkok',
      action: 'Emergency medical services in Bangkok',
    },
    {
      title: 'Specialist consultation',
      label: 'for chronic back pain',
      action: 'Specialist consultation for chronic back pain',
    },
  ];
...
function PureStopButton({ onStop }: { onStop: () => void }) {
  return (
    <Button
      ...
      className="rounded-full p-1.5 h-fit border border-medifly-teal text-white"
      onClick={(event) => {
        event.preventDefault();
        onStop();
      }}
    ...
    <div className={cn("relative w-full flex flex-col gap-4", className)}>
      ...
      {(attachments.length > 0 || uploadQueue.length > 0) && (
        <div
          ...
          className="flex pt-2.5 flex-row gap-3 overflow-x-auto items-end pb-2 pl-1"
        >
          {attachments.map((attachment) => (
            <div key={attachment.url || attachment.name} className="relative group">
                ...
                <Button
                  variant="destructive"
                  size="icon"
                  className="absolute -top-2 ri-ght-2 h-5 w-5 rounded-full p-0 flex items-center justify-center z-20 opacity-0 group-hover:opacity-100 transition-opacity"
      ...
      <Textarea
        ...
        ref={textareaRef}
        placeholder="Describe your health concern or ask about medical services..."
        value={input}
        onChange={handleInput}
        className={cn(
          'min-h-6 max-h-[calc(75dvh)] overflow-y-auto resize-none rounded-2xl text-base! pb-10',
          'bg-white  focus:border-medifly-teal', 
          className,
        )}
        style={{color: 'black'}}
        rows={1}
